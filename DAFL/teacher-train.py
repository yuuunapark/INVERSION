...
parser.add_argument('--data', type=str, default='./cache/data/') ### 경로 수정: '/cache/data/'에서 './cache/data/' (절대경로에서 상대경로로 )
parser.add_argument('--output_dir', type=str, default='./cache/models/') ### 경로 수정: '/cache/models/'에서 './cache/models/'
...
### train/test 마친 후, teacher network 저장
def main():
    if args.dataset == 'MNIST':
        epoch = 10
    else:
    ...
    for e in range(1, epoch):
        train_and_test(e)
    torch.save(net,args.output_dir + 'teacher.pth')
