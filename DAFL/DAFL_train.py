...
parser.add_argument('--data', type=str, default='./cache/data/') # './cache'와 'cache'는 같은 것
...
### 단일 GPU가 아닌 여러 GPU에서 모델을 병렬로 실행
...
### epoch에 따라서, optimizer의 learning rate를 조정한다. 
...
for epoch in range(opt.n_epochs):
    ...
    for i in range(120):
        ...
        ### 평균이 0, 표준편차가 1인 정규분포에서 난수 생성: random noise 벡터 z 생성
        ...
        ### output logit # feature map 
        pred = outputs_T.data.max(1)[1]

        ### loss 1) activation loss
        ...
        ### loss 3) information_entropy loss
    ...
    ### 모델 평가 과정
    ### torch.no_grad는 gradient를 계산하지 않아 메모리 사용이 최적화된다.
    ### student의 output과 test data의 label을 비교하여 loss를 계산한다.
    with torch.no_grad():
    ...
    ### avg_loss 변수를 테스트 데이터셋의 크기(len(data_test))로 나눈 후 다시 avg_loss에 저장
